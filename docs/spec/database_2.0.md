...
DBMS (Production): PostgreSQL 14+  
DBMS (Testing): SQLite (Local database for testing)  
...
erDiagram
    ...
    SERVICES ||--o{ SERVICE_USER : "can be performed by"
    USERS ||--o{ SERVICE_USER : "specializes in"
...
Table: service_user  
Pivot / junction table between services and users (executors). Defines which services an employee is qualified to perform.

* id (BIGSERIAL, PK) — Unique row identifier.
* service_id (BIGINT, FK -> services.id ON DELETE CASCADE) — Service reference.
* user_id (BIGINT, FK -> users.id ON DELETE CASCADE) — User reference.
* created_at, updated_at (TIMESTAMP(0)) — System timestamps.
* Indexes: Unique composite index on `[service_id, user_id]`.
